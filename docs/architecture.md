...
The legacy combined CLI remains only while inbound SMTP, IMAP, POP3, and ManageSieve callers are migrated checkpoint by checkpoint. Its remaining presence keeps this branch non-deployable until the final boundary tests prove that Gateway is the only presentation host and Application Worker is the only application-logic host.
