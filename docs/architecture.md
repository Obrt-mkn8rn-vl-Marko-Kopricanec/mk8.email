...
The legacy combined CLI remains only while IMAP and POP3 callers are migrated checkpoint by checkpoint. POP3's CRLF normalization, wire-size calculation, TOP slicing, and dot-stuffing now live in dependency-free MailWire so the forthcoming Gateway listener and Worker-side mailbox snapshot can share exact octet semantics without moving socket handling into Application. Its remaining presence keeps this branch non-deployable until the final boundary tests prove that Gateway is the only presentation host and Application Worker is the only application-logic host.

Gateway's POP3 presentation will hold an atomic PostgreSQL maildrop lease per authenticated user rather than keeping an Application Worker call or database connection open for the lifetime of a TCP session. Every lease has a unique owner token, renews on activity, and can be reclaimed after expiry; release and renewal cannot affect a replacement owner's session. POP3 listener cutover to use this lease remains unfinished.

Application Worker now exposes typed POP3 password/OAuth authentication, INBOX snapshot, message-content read, and transactional deletion operations. It never receives a POP3 socket or renders protocol responses; the legacy listener still performs those actions until the Gateway cutover.
